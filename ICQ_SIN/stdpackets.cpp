...
DWORD CIcqProto::icq_sendGetStealthAwayMsgServ(HANDLE hContact, DWORD dwUin, int type, WORD wVersion)
{
  icq_packet packet;
  DWORD dwCookie;
  ...
  packFNACHeader(&packet, ICQ_LOCATION_FAMILY, ICQ_LOCATION_REQ_USER_INFO, 0, (WORD)dwCookie);
  packWord(&packet, 0x03);
...
void CIcqProto::icq_sendAwayMsgReplyServ(DWORD dwUin, DWORD dwMsgID1, DWORD dwMsgID2, WORD wCookie, WORD wVersion, BYTE msgType, char** szMsg)
{
	...
	if (validateStatusMessageRequest(hContact, msgType))
	{
		...
		if (szMsg && *szMsg)
		{
			...
			if(m_iStatus == ID_STATUS_ONLINE && msgType == MTYPE_AUTOONLINE)
				msgType = MTYPE_AUTOAWAY;//nasty workaround, icq6 doesn't understand mtype_autoonline
