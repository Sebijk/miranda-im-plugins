...
int CIcqProto::SetStatusNote(const char *szStatusNote, DWORD dwDelay, int bForce)
{
  ...
  { // check if the status note was changed and if yes, create thread to change it
    ...
    { // status note was changed
	  ...
	  //don't show it on profile
	  if(gbNoSMSProfileEnabled)
		  setStatusNoteText = null_strdup("");
...
void CIcqProto::makeUserOffline(HANDLE hContact)
{
	...
	setInvisibleIcon(hContact, FALSE);
}

INT_PTR CIcqProto::SetInvis(WPARAM wparam,LPARAM lparam)
{
...
INT_PTR CIcqProto::SetVis(WPARAM wparam,LPARAM lparam)
{
...
/*
BOOL checkPaket(unsigned char *buf, WORD datalen)
{
	DWORD dwUIN;
    uid_str szUID;
    WORD wTLVCount;
    ...
    if (!unpackUID(&buf, &datalen, &dwUIN, &szUID)) return FALSE;
	...
	if((wTLVType == 0x001D) && (wTLVLen > 0))
	{
		unpackWord(&buf, &wSign);
		unpackByte(&buf, &bSign);
		datalen -= 3;

		if((wSign == 0x0001) && (bSign == 0x01))
			return FALSE;
		else if ((wSign == 0x0002) && (bSign == 0x04))
			return TRUE;
	}
...
*/
/*
int processWindowEvent(WPARAM wParam, LPARAM lParam)
{
  MessageWindowEventData* mData = (MessageWindowEventData*)lParam;

  if(mData->uType == MSG_WINDOW_EVT_CLOSE)
  {
	  DWORD dwUin = ICQGetContactSettingDword(mData->hContact, UNIQUEIDSETTING, 0);

      for(msgcontact=0;StatusList[msgcontact].dwUin != dwUin; msgcontact++)
      {}

	  if(StatusList[msgcontact].dwContacted == WAS_CONTACTED)
	  {
	      sendTypingNotification(mData->hContact,MTN_WINDOW_CLOSED);
		  StatusList[msgcontact].dwContacted = 0;
	  }
  }
...
void setChatStatus(DWORD dwUin)
{
	...
	StatusList[msgcontact].dwContacted = WAS_CONTACTED;
}

*/
/* // sets note to nothing

    icq_packet packet;

  	// Pack data in packet
    serverPacketInit(&packet, 22);
	packFNACHeader(&packet, ICQ_SERVICE_FAMILY, ICQ_CLIENT_SET_STATUS);
	packWord(&packet, 0x1D);              // TLV 1D
	packWord(&packet, 0x08);              // TLV length
    packWord(&packet, 0x02);              // Item Type
	packWord(&packet, 0x00);
	packWord(&packet, 0x0e);
	packWord(&packet, 0x00);
