...
// Last change on : $Date: 2009-05-09 00:07:25 +0200 (Sa, 09 Mai 2009) $
...
void CIcqProto::handleMessageTypes(DWORD dwUin, DWORD dwTimestamp, DWORD dwMsgID, DWORD dwMsgID2, WORD wCookie, WORD wVersion, int type, int flags, WORD wAckType, DWORD dwDataLen, WORD wMsgLen, char *pMsg, int nMsgFlags, message_ack_params *pAckParams)
{
  ...
  BOOL bThruDC = (nMsgFlags & MTF_DIRECT) == MTF_DIRECT;
	...
	case MTYPE_AUTOFFC:
	  ...
	  //make dnd message readable for icq6
      if ((type == MTYPE_AUTOBUSY && m_iStatus == ID_STATUS_DND))
		  type = MTYPE_AUTODND;

      if ((type == MTYPE_AUTOAWAY && m_iStatus == ID_STATUS_ONLINE))
		  type = MTYPE_AUTOONLINE; 
		  
	  szMsg = MirandaStatusToAwayMsg(AwayMsgTypeToStatus(type));
			if (szMsg)
			{
				rate_record rr = {0};
...
void CIcqProto::handleRecvServMsgError(BYTE *buf, WORD wLen, WORD wFlags, DWORD dwSequence)
{
	...
	{ // all packet cookies from msg family has command 0 in the queue
		...
		case 0x0004:     // Recipient is not logged in (resend in a offline message)
	        ...
	        if(pCookieData->bMessageType == MTYPE_ADDED)
		        setInvisibility(dwUin, hContact); //invisibility detected
		...
		if (pCookieData && (pCookieData->nAckType == ACKTYPE_SERVER))
		{
			if ((hContact != NULL) && (hContact != INVALID_HANDLE_VALUE))
			{
				...
				case MTYPE_FILEREQ:
					...
					// Note 2: The cookiedata is NOT a cookie_message_data*, it is a
