...
// Revision       : $Revision: 9388 $
// Last change on : $Date: 2009-04-10 09:00:55 +0200 (Fr, 10 Apr 2009) $
...
/*
	...
	{
	case 0x0100:
		ShowPopUpMsg(hContact, NickFromHandleUtf(hContact), "is Online as invisible", LOG_NOTE);
		break;
	case 0x0101:
		ShowPopUpMsg(hContact, NickFromHandleUtf(hContact), "is Away as invisible", LOG_NOTE);
		break;
	case 0x0105:
		ShowPopUpMsg(hContact, NickFromHandleUtf(hContact), "is N/A as invisible", LOG_NOTE);
		break;
	case 0x0111:
		ShowPopUpMsg(hContact, NickFromHandleUtf(hContact), "is Busy as invisible", LOG_NOTE);
		break;
	default:
		break;
	}
*/
    ...
    setInvisibleIcon(hContact, FALSE);//remove inv icon
}
...
//  BOOL check = FALSE;
  ...
  hContact = HContactFromUID(dwUIN, szUID, NULL);
	...
	NetLog_Server("'went offline'-message received from %s.", strUID(dwUIN, szUID));
/*
	if(hContact != INVALID_HANDLE_VALUE && ICQGetContactStatus(hContact) == ID_STATUS_OFFLINE)
	{
		setInvisibility(dwUIN, hContact); //invisibility detected
		check = TRUE;
	}
*/
  ...
  if (hContact != INVALID_HANDLE_VALUE)
  {
	  if((getSettingWord(hContact, "Status", 0) != ID_STATUS_INVISIBLE) &&
		  (getSettingWord(hContact, "Status", 0) != ID_STATUS_OFFLINE))
	  {
          checkInvisibility(0, hContact, 3); //check if user really went offline
	  }
	  else
	  {	  if(!strncmp(getSettingStringUtf(hContact, "MirVer", ""),"QIP", 3))
		      checkInvisibility(0, hContact, 4);
	      else
			  checkInvisibility(0, hContact, 0); //check if user really went offline
	  }
  }

  // Skip contacts that are not already on our list
  if ((hContact != INVALID_HANDLE_VALUE) && !CheckContactCapabilities(hContact,WAS_FOUND))
  {
    NetLog_Server("%s line.", strUID(dwUIN, szUID));
