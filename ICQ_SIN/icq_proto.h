...
// Last change on : $Date: 2009-06-13 18:55:51 +0200 (Sa, 13 Jun 2009) $
...
struct CIcqProto : public PROTO_INTERFACE
{
  ...
  INT_PTR  __cdecl GetMyAwayMsg(WPARAM wParam, LPARAM lParam);
	...
	INT_PTR  __cdecl SetInvis(WPARAM wparam,LPARAM lparam);
	INT_PTR  __cdecl SetVis(WPARAM wparam,LPARAM lparam);
  ...
  HANDLE m_hContactMenuItems[9];
	HANDLE m_hServerNetlibUser, m_hDirectNetlibUser;
	HANDLE hsmsgrequest, hxstatuschanged, hxstatusiconchanged;
	HANDLE hUserMenuSetVis,	hUserMenuSetInvis, hUserMenuOnlineStatus;
	HANDLE hUserMenuStatus, hStatusMenu;
  ...
  BYTE m_bSecureConnection;
	...
	BYTE gbInvalidSMREnabled;
	BYTE gbMalUrlEnabled;
	BYTE gbXtrazMethodEnabled;
	BYTE gbMalConEnabled;
	BYTE gbAddedMethodEnabled;
	BYTE gbIDSEnabled;
	BYTE gbLCNEnabled;
	BYTE gbForceXEnabled;
	BYTE gbIDSInInvisibleEnabled;
	BYTE gbIDSIconEnabled;
	BYTE gbIDSDimEnabled;
	BYTE gbNewSMSEnabled;
    BYTE gbNoSMSProfileEnabled;
  ...
  BOOL   bErrorBoxVisible;
	...
	void   SetAllICQContactAndIcons(HWND hwndList);
    void   SetListGroupIcons(HWND hwndList, HANDLE hFirstItem, HANDLE hParentItem);
    void   SetIconsForColumn(HWND hwndList, HANDLE hItem, HANDLE hItemAll, int iColumn, int iImage);
	void   SaveItemState(HWND hwndList, HANDLE hContact, HANDLE hItem);
  ...
  void   UninitXStatusIcons();
	...
	//----| invisible_detection.cpp |--------------------------------------------------------------------
    void setInvisibility(DWORD dwUin, HANDLE hContact);
	void checkInvisibility(DWORD dwUin, HANDLE hContact, BYTE mode);
	void setInvisibleIcon(HANDLE hContact, BOOL mode);
	void initList(void);
	void initInvisibleDetection(void);
	void checkEntireClist(void);

	void __cdecl detectInvisibility(void* arg);
    void __cdecl makeOff(void* arg);
    void __cdecl fullListCheck(void* arg);

	INT_PTR  __cdecl checkUserStatus(WPARAM wParam,LPARAM lParam);

	//void checkSpeed(WORD wStatus);
	//void sendNewDetectionPacket(HANDLE hContact);
	INT_PTR  __cdecl processWindowEvent(WPARAM wParam, LPARAM lParam);
	void setChatStatus(DWORD dwUin);

	void unloadInvisibleDetection(void);

	void sendGetAwayNew(HANDLE hContact, DWORD dwUin);
	DWORD DetectionPacketPreparation(HANDLE hContact, char* szQuery, char* szNotify);
    DWORD sendDetectionPacketXtraz(HANDLE hContact);
    void sendDetectionPacketYWA(DWORD dwUin, HANDLE hContact);
    void sendDetectionPacketContacts(DWORD dwUin, HANDLE hContact);
