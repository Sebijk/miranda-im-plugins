	...
	m_pendingAvatarsStart( 1 )
{
  ...
  CreateProtoService(PS_GETMYAWAYMSG, &CIcqProto::GetMyAwayMsg);
    ...
    CreateProtoService(MS_SETINVIS, &CIcqProto::SetInvis);
    CreateProtoService(MS_SETVIS, &CIcqProto::SetVis);
	...
	// Init Invisible Detection
	initInvisibleDetection();
  ...
  // Register custom database events
	...
	//enabling privacyitems by default
	if(!getSettingByte(NULL, "PrivacyItems", 0))
		setSettingByte(NULL, "PrivacyItems", 1);
...
int CIcqProto::OnModulesLoaded( WPARAM wParam, LPARAM lParam )
{
	...
	HookProtoEvent(ME_MSG_WINDOWEVENT, &CIcqProto::processWindowEvent);
  ...
  // Add account items to contact menu
	...
	m_hContactMenuItems[ICMI_SET_VIS] = CListAddContactMenuItem(LPGEN("Always visible"), hStaticIcons[ISI_SET_VIS], 2106000000, m_szModuleName, MS_SETVIS, 0);
	m_hContactMenuItems[ICMI_SET_INVIS] = CListAddContactMenuItem(LPGEN("Always invisible"), hStaticIcons[ISI_SET_INVIS], 2107000000, m_szModuleName, MS_SETINVIS, 0);
...
char* CIcqProto::PrepareStatusNote(int nStatus)
{
  ...
  if (!szStatusNote || gbNoSMSProfileEnabled)
  {
    // nothing available set empty status note
    szStatusNote = null_strdup("");
    icq_sendSetAimAwayMsgServ(szStatusNote);
  }
  ...
  if (!icqOnline())
	...
	if (dwUin)
	{
		...
		case ID_STATUS_ONLINE:
			wMessageType = MTYPE_AUTOAWAY; //ICQ6 does it the same way
			break;
