...
void CIcqProto::handleServiceFam(BYTE *pBuffer, WORD wBufferLength, snac_header *pSnacHeader, serverthread_info *info)
{
	...
	case ICQ_SERVER_RATE_CHANGE:
		...
		if (wBufferLength >= 2)
		{
	       ...
	       //checkSpeed(wStatus); //variable FLC scanspeed to avoid server-kick (rate-warnings)
		}
...
{ 
	...
	if (gbNewSMSEnabled && !gbNoSMSProfileEnabled)
		wAdditionalData += 16; //for new status message

    wAdditionalData += 16; //for Icqj S!n cap
	...
	if(gbNewSMSEnabled && !gbNoSMSProfileEnabled)
	{
		packNewCap(&packet, 0x010A);    // Tells the server we support new status messages
	}
    ...
    packBuffer(&packet, capIcqJSin, 4); //icqj s!n cap
...
/*  packDWord(&packet, 0x67361515); /// icq lite audio chat Xtraz "ICQTalk" plugin
  ...
  packDWord(&packet, 0x408ea041);*/
	...
	//wrote this way, that icqj s!n can be detected the old way
...
void CIcqProto::handleServUINSettings(int nPort, serverthread_info *info)
{
    ...
    WORD wSessionDataLen = (wStatusNoteLen ? wStatusNoteLen + 4 : 0) + 4 + wStatusMoodLen + 4;
		...
		if(gbNewSMSEnabled)
		    packTLVWord(&packet, 0x0008, 0x0a06);       // TLV 8: Error code // support new status message -> 0x0a06 !!!
		else
		    packTLVWord(&packet, 0x0008, 0x0000);       // TLV 8: Error code // support new status message -> 0x0a06 !!!
		...
		packDWord(&packet, gbUnicodeCore ? 0x80000000 : 0x80000000); // Abused timestamp //wrote this way that icqj s!n can be detected the old way
...
*/ //no it doenst work, status handles were changed thiso ne is obsolete and causes troubles
}
