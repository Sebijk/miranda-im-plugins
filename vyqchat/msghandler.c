...
/*
		if(NetWkstaGetInfo(NULL, 100, (LPBYTE *) &wksta_info100) == NERR_Success) {
			...
			r_netgroup_name = _strdup("");
		}
*/
		r_netgroup_name = mir_strdup("");
		...
		mir_free(r_autoanswer);
		mir_free(r_chanlist);
		mir_free(r_user_name);
		mir_free(r_computer_name);
		mir_free(r_netgroup_name);
	}
	mir_free(src);
	mir_free(dst);
}
	...
	const char * r_src, const char * r_dst)
{
	...
	if(!user_is_my_nickname(src) && user_is_my_nickname(dst)) {
		...
		if(hContact) {
			...
			if (ServiceExists(MS_SHAKE_CLIST))
				CallService(MS_SHAKE_CLIST, 0, 0);

			if (ServiceExists(MS_SHAKE_CHAT))
				CallService(MS_SHAKE_CHAT, (WPARAM)hContact, 0);
