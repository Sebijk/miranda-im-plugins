	...
	void * key, void * value, void * user_data)
{
	...
	if(!entry->alive) {
		...
		info->dead_keys[info->dead_key_count++] = _strdup((char*)key);
	} else {
	...
	int add_to_log)
{
	...
	hashtable_insert(s_userlistHash, _strdup(name), entry);
...
void userlist_name_change(const char * user_name, const char * new_user_name)
{
	...
	if(entry) {
		...
		hashtable_insert(s_userlistHash, _strdup(new_user_name), entry);
