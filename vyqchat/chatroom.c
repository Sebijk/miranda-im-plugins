...
#include "hashtable/strhashfunc.h"
	...
	const char * channel, int connecting_to_net)
{
	...
	if(entry == NULL) {
		...
		entry->topic = _strdup("");
		hashtable_insert(s_channelHash, _strdup(channel), entry);
	}
	...
	if(s_fHaveChat) {
		...
		if(!CallService(MS_GC_NEWSESSION, 0, (LPARAM)&gcw)) {
			...
			CallService(MS_GC_EVENT, (WPARAM)SESSION_INITDONE, (LPARAM)&gce);
			CallService(MS_GC_EVENT, (WPARAM)SESSION_ONLINE, (LPARAM)&gce);

			mir_free(l_user_nickname);
		}

		/* free channel name '#chan' we've alloced */
		mir_free(l_channel_name);
		mir_free(l_current_topic);
	}
}
	...
	if(s_fHaveChat && topic_changed
		...
		if(notify_network) {
			...
			msgloop_send(
				...
				0);
			mir_free(r_channel);
			mir_free(r_new_topic);
		}

		mir_free(l_new_topic);
	}
}
	...
	const char * channel, void * enum_fn_data)
{
	...
	chatroom_setup_event(&gcd, &gce, channel, GC_EVENT_CHUID);
	gce.pszUID = l_user_name;
	gce.pszText = l_new_user_name;
	gce.pszStatus = CHATROOM_NORMAL_STATUS;
	gce.bIsMe = TRUE;
	...
	const char * text, int action_text)
{
	...
	if(s_fHaveChat && chanlist_contains(*user_p_chanlist(), channel)) {
		...
		gce.bIsMe = user_name == user_nickname();
		//gce.bAddToLog = TRUE;
		CallService(MS_GC_EVENT, 0, (LPARAM)&gce);

		mir_free(l_user_name);
		mir_free(l_escaped_text);
		free(escaped_text);
	}
}
