	...
	HANDLE hContact, WPARAM * p_wparam)
{
	...
	text = _strdup(dbv.pbVal + sizeof(WPARAM));
	...
	HANDLE hContact, enum contact_property_enum property)
{
	...
	prop_name = contact_prop_name_by_enum(property);
	ASSERT_RETURNVALIFFAIL(VALIDPTR(prop_name), _strdup("unknown"));

	value = NULL;
	if(!db_get(hContact, VQCHAT_PROTO, prop_name, &dbv)) {
		if(dbv.type == DBVT_ASCIIZ) {
			value = _strdup(dbv.pszVal);
		}
		DBFreeVariant(&dbv);
	}
	if(value == NULL)
		value = _strdup("(unknown)");
