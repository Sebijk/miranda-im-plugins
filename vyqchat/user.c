...
int user_set_nickname(const char * new_nickname, int store_in_db)
{
	...
	if(store_in_db)
		db_string_set(NULL, VQCHAT_PROTO, "Nick", util_utf2loc(new_nickname));
	...
	/* change nickname */
	free(s_userNickname);
	s_userNickname = _strdup(new_nickname);
