...
static BOOL CALLBACK options_userinfo_dlgproc(HWND hdlg, UINT nmsg, WPARAM wp, LPARAM lp)
{
	switch(nmsg) {
	case WM_COMMAND:
...
static BOOL CALLBACK options_useropt_dlgproc(HWND hdlg, UINT nmsg, WPARAM wp, LPARAM lp)
{
...
static void options_load_settings()
{
	...
	if(!rc && dbv.type==DBVT_ASCIIZ) {
		if(strlen(dbv.pszVal) != 0) {
			/* ok, got name from db */
			user_set_nickname(util_loc2utf(dbv.pszVal), FALSE);
		}
		DBFreeVariant(&dbv);
	}
